...
router.post('/users/friend', cors(corsOptions), auth, async (req, res) => {  //find user by username
    const username = req.params.username
    const users = []
    try {
        const users1 = await Users.find({"name": username})
        const users2 = await Users.find({"Lname": username})

        if (!users1 && !users2 ) {
            return res.status(404).send({"error":"Pas d'utilisateur sous ce nom"})
        } else if (users1) {
            users1.forEach(user => {
                users.push(user)
            });
